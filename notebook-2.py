...
def _():
    ...
    first_name = "Josh" # FIXME: assign a name to the first_name variable

    # We define integer and floating point numbers with no quotes
    # year = 2026

    year = 2005 # FIXME: assign a number (no quotes) to the year variable
...
def _():
    ...
    _time_minutes = 42
    _time_seconds = _time_minutes * 60 # FIXME
...
def _():
    _mass = 1.6
    _light_speed = 3.0e8
    _energy = _mass * (_light_speed)**2 # FIXME (using E=mc^2 but with correct Python operators)
...
@app.cell
def _():
    _answer = 5978 # FIXME: write a number as a string (with quotes)
...
@app.cell
def _(masses_list):
    print(f"minimum of values is {min(masses_list)} amu.")
    return
...
def _():
    ...
    dy = coords1[1] - coords2[1] # FIXME
    dz = coords1[2] - coords2[2] # FIXME
    return dx, dy, dz
...
def _(asdict):
    def _():
        ...
        @dataclass
        class Reaction:
           solvent : str # FIXME: write a class that stores solvent (string), 
           temperature : float
           time : int # and temperature (float), time (integer)
        _Benzoin_Cond = Reaction(solvent= 'DCM', temperature= 25.0, time= 24)
        asdict(_Benzoin_Cond)
        print(_Benzoin_Cond)
        return
    _()
    return
